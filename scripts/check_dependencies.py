...
def check_transformers():
    ...
    print("\n[CHECK] Transformers version...")
    output = run_cmd(f'"{VENV_PYTHON}" -c "import transformers; print(transformers.__version__)"')
    if output:
...
def check_llamafactory():
    ...
    output = run_cmd(f'"{VENV_PYTHON}" -c "import llamafactory; print(llamafactory.__version__)"')
    if output:
...
def check_torch():
    ...
    output = run_cmd(f'"{VENV_PYTHON}" -c "import torch; print(torch.cuda.is_available())"')
    if output == "True":
        print("  CUDA: Available")
        output = run_cmd(f'"{VENV_PYTHON}" -c "import torch; print(torch.cuda.get_device_name(0))"')
        if output:
...
def main():
    ...
    if issues:
        ...
        if fix_mode:
            ...
            if llamafactory_ok and transformers_ok:
                print("\nAll issues fixed!")
            else:
