...
def install_cuda_for_monitoring(venv_path):
    ...
    try:
        ...
        print("\n[1/3] Uninstalling existing PyTorch packages...")
        subprocess.run(
            [
                str(pip_exe),
                "uninstall",
        ...
        # Use CUDA 12.1 for better compatibility
        print("\n[2/3] Installing PyTorch with CUDA 12.1 support...")
        result = subprocess.run(
            [
                str(pip_exe),
                "install",
        ...
        # Install torchdata with compatible version
        print("\n[3/3] Installing torchdata (required for LlamaFactory)...")
        subprocess.run(
            [str(pip_exe), "install", "torchdata>=0.10.0,<=0.11.0"],
            capture_output=True,
            text=True,
        )

        venv_python = (
            get_pip_exe(venv_path).parent / "python.exe"
            if platform.system() == "Windows"
            else get_pip_exe(venv_path).parent / "python"
        )
        ...
        if "True" in check_result.stdout:
            ...
            print("[WARN] CUDA still not available within venv. Check NVIDIA drivers.")
            print("[INFO] If GPU is detected by nvidia-smi but not PyTorch:")
            print("       - Try restarting your terminal/command prompt")
            print("       - Or run: nvidia-smi to verify driver installation")
            return False
        ...
        print(
            f"  {pip_exe} install torch torchvision torchaudio --index-url https://download.pytorch.org/whl/cu121"
        )
        print(f"  {pip_exe} install torchdata>=0.10.0,<=0.11.0")
        return False
